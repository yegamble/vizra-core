...
| Every 503 on the setup surface logs its cause once, redacted, with the request id — the claim-status GET and the unclaimed guard included, which used to log only the canned message (sentinel S-0006). A request whose OWN context has ended (the client hung up) is answered 503 and logged at no level at all: not as "could not reach the database", and not by the shared error handler (sentinel S-0005, RULES R9). The test is the request context, not `errors.Is(err, context.DeadlineExceeded)`, because a pgx connect timeout wraps DeadlineExceeded and IS an outage; `ownerclaim.unavailable` wraps its cause with `%w` so the cause stays inspectable (RULES R18) | `s.unavailable`, `errorHandler`; `TestEverySetup503LogsItsCause` (claim-status, the guard, and a POST with no pool), `TestACancelledClaimIsNotLoggedAsADatabaseOutage` (cancelled during the claimed check, and inside the claim transaction while the redeem waits on a held row lock) |
| The owner-claim validator never accepts an input the database then refuses: the email expression is the DDL's own literal (`TestValidatorsMatchTheMigration` compares it byte for byte, the username too), and because PostgreSQL's `[:space:]` follows the libc locale while Go's is ASCII-only, the validator ALSO refuses every Unicode space and control character and invalid UTF-8 — strictly more than any locale's `[:space:]`. A value `text` cannot hold (NUL, SQLSTATE 22021) is mapped to 400 as a backstop (sentinel S-0003, RULES R17). **At exactly this strength:** the corpus test runs one locale (the CI image's `en_US.utf8`); the "strictly more" argument is what covers the others | `TestEveryEmailTheValidatorAcceptsTheDatabaseAccepts` (one corpus through Go and through users_email_shape's CHECK expression read from 0005's bytes, evaluated by PostgreSQL), `TestAnEmailTheDatabaseCannotHoldIsA400NotA500`, `TestTheEmailValidatorRefusesEveryLocaleDependentSpaceAndControl`, `TestClaimErrorMapping` (22021) |
...
| A `setup.owner_claim.succeeded` audit row names the token generation it redeemed, as 0005 promises, and the database refuses one that does not: migration 0006's `audit_events_claim_succeeded_names_generation` CHECK (`NOT VALID`, so rows written under 0005 are not re-checked; every row inserted since is) (sentinel S-0007) | `TestTheSucceededRowNamesTheConsumedGeneration`, `TestTheDatabaseRefusesASucceededRowWithoutAGeneration` |
