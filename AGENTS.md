...
| **Zero** argon2id derivations on every non-201 claim path — a wrong token, a malformed field, a 415/413/429, an already-claimed instance, and a token that is CORRECT but expired, consumed or superseded. The ONE exception, stated because it is real: a claimant that passed every check may hash and then lose the redeem to a concurrent claimant, so a true race can cost one derivation for a 409. | `internal/credential` (injectable hasher with a derivation counter) + the liveness pre-check in `ownerclaim.Claim`; `TestNoPasswordHashingOccursWithoutAValidToken`, `TestACorrectButDeadTokenCostsNoDerivation` |
...
| A permanently closed endpoint is not an unauthenticated writer into the undeletable audit table | the claimed bit short-circuits from the monotonic cache before the pool is touched, and the `already_claimed` refusal writes no row; `TestRepeatedClaimsOnAClaimedInstanceDoNotGrowTheAuditTrail` |
| A database outage answers 503, never 500 and never "token not accepted" | `ownerclaim.ErrUnavailable` wraps every infrastructure failure; `TestClaimErrorMapping`, `TestOwnerClaimAnswers503WhenTheDatabaseIsDown`, `TestEveryStatusTheContractDeclaresIsProducedAndNoOtherIs` |
