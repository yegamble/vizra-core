...
- Every TEXT check runs on the pinned read set **before make is invoked**:
  ...
  word; `.DEFAULT_GOAL` is harmless) or `.EXTRA_PREREQS`, a pattern rule,
  literal `-`/`+` prefixes, `|| true` suffixes and `$(MAKE)` on gate recipes,
  duplicate and conditional gate targets, a gate closure target with no
  explicit rule or not declared `.PHONY` — GNU make searches no implicit,
  pattern or `.DEFAULT` rule for a phony target, so every recipe the gate
  closure reaches is one the text reading scans — and the `?=` environment
  variables.
- What only make can resolve is refused AFTER make has run on the pinned
  ...
  missing from make's own `.PHONY` list, likewise); a `-`/`+` prefix a LEADING variable expands to (make applies the
...
`scripts/testdata/fakedocker/` (6), `scripts/testdata/makeguard/` (57, each with
