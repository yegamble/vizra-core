...
| The API leaks no credential into its OWN log whichever slog handler it was built with (`New` falls back to `slog.Default()`, which redacts nothing) — every value at every `internal/httpapi` log site is a literal or `obs.Redact(...)`, the 500 path's cause, path, method and request id included | behaviour: `TestTheErrorHandlerRedactsTheCauseOfA500` drives the shared error handler with a plain `slog.JSONHandler` and a cause carrying a DSN password and a presigned signature and credential, on a path carrying an API key. Coverage: `TestEveryLogSiteInTheAPIIsRedacted` parses every non-test file in the package, requires a literal message, literal keys (so no `slog.Attr` constructor) and a literal or `obs.Redact(...)` value, refuses the standard `log` package, `fmt.Print*`/`Fprint*`, `print`/`println` and `os.Stdout`/`os.Stderr`, and fails if the count of **6** sites drifts; `TestTheLogSiteCheckerCatchesPlantedUnredactedCalls` plants each shape and requires the checker to report it (security review of core #8, N-7 / F-1) |
...
| A request whose context has ENDED is not a cache failure: the limiter answers it `(false, 0)` and touches neither the cache, the in-process counter nor the degraded flag — checked before the command and again when the command fails, since a context that ends mid-command surfaces as an `Exec` error. Before this, a client hanging up flipped `/readyz` to degraded and charged the fallback for a dead request (limiter review L-2) | `FallbackLimiter.Allow`; `TestACancelledRequestDoesNotFlipTheLimiterToDegraded` (with a live-context control that DOES flip, against the same dead cache), `TestARequestCancelledMidCommandDoesNotFlipTheLimiterToDegraded`, `TestACancelledRequestDoesNotChargeTheInProcessLimiter` |
