...
- Every TEXT check runs on the pinned read set **before make is invoked**:
  ...
  `$`-named prerequisite on a gate closure rule (#11 fix round 1, cross-check
  X-1, and B5c), literal `-`/`+` prefixes, `|| true` suffixes and `$(MAKE)` on
  gate recipes, duplicate and conditional gate targets, a gate closure target
...
- What only make can resolve is refused AFTER make has run on the pinned
  ...
  missing from make's own `.PHONY` list, likewise); every closure target's
  recipe as make's database holds it, held to the same literal checks — the
  backstop for a recipe make attaches some other way, e.g. a rule whose
  target NAME make computes (fixture `computed-target-recipe`); a `-`/`+` prefix a LEADING variable expands to (make applies the
