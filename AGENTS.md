...
| `obs.Redact` removes exactly these value forms and no others: URL userinfo (`scheme://user:pw@`, an empty user included — the `redis://:pw@` form); `Cookie:`/`Set-Cookie:` header values; credential-named `key=value` pairs — a key that is `key`, `sig`, `signature`, `AWSAccessKeyId`, `X-Amz-Signature` or `X-Goog-Signature`, or that CONTAINS `password`, `passwd`, `secret`, `token`, `session`, `credential` or `api_key`/`apikey` — as URL query parameters, keyword/value DSNs (`password=…`, `password='…'`) and cookie pairs; `Bearer` tokens; and `vzk_` API keys. The `obs.NewLogger` handler additionally replaces the whole value of an attribute whose KEY is secret-named (`password`, `token`, `session_id`, `dsn`, …) and runs `Redact` over every other string. **Not covered, at exactly this strength:** raw private metadata (there is no pattern for it), a secret under a JSON key (`"password":"…"`), a URL password containing a raw `/` or `@`, a credential-shaped value with no recognisable key or prefix, and anything logged through another handler by a call site that does not call `Redact` itself | `internal/obs`: `TestRedactCoversEveryCredentialForm` (one row per form), `TestRedactLeavesLookalikesAlone` (the false-positive control: `keyboard`, `monkey=`, `hotkey=`, pgx's "password authentication failed", `https://host:443/@alice`), `TestRedactionOfEveryValueClass` |
...
| Every `"error"` attribute the worker logs passes `safeError` (= `truncate(obs.Redact(…))`) AT THE CALL SITE, whichever slog handler it was built with — redaction is at the call site, not in the process wiring. What that removes is `obs.Redact`'s pattern set, nothing broader | behaviour: `TestAWorkerWithAPlainHandlerLogsNoCredentials` drives the three handler-error branches with a plain `slog.TextHandler`. Coverage: `TestEveryErrorLogSiteInTheWorkerIsRedacted` parses `worker.go` and asserts **all 11** `"error"` attributes are `safeError(...)`, and fails if the count drifts |
| Every value at every `internal/httpapi` log site, and at `internal/search`'s two fallback log sites, passes `obs.Redact` AT THE CALL SITE, so what those sites remove does not depend on the handler the process installed (`httpapi.New` and `search.NewService` fall back to `slog.Default()`, which redacts nothing). What that removes is `obs.Redact`'s pattern set (the row above) — nothing broader. **Residuals, review-only:** a handler PANIC is written by net/http to its own stderr logger, unredacted (there is no Recover middleware and no `ErrorLog`; the checker refuses a non-literal `panic` in `internal/httpapi`, but a panic raised deeper is not seen); a logger handed to a helper in ANOTHER package is not followed; other log sites in the api process are covered only where a test below names them | behaviour: `TestTheErrorHandlerRedactsTheCauseOfA500` and `TestTheErrorHandlerRedactsEveryCredentialFormInA500` (one 500 per form the B3 verifier measured leaking, under a plain `slog.JSONHandler`); `TestTheFallbackLogLinesAreRedacted` (a real `*url.Error` quoting `VIZRA_SEARCH_URL`'s userinfo and `?password=`, under a plain text handler). Coverage: `TestEveryLogSiteInTheAPIIsRedacted` parses every non-test file in `internal/httpapi` and requires a literal message, literal keys (so no `slog.Attr` constructor) and a literal or `obs.Redact(...)` value at each slog call. It refuses BY NAME the standard `log` package, `fmt.Print*`/`Fprint*`, `print`/`println`, `os.Stdout`/`os.Stderr`/`os.NewFile`, `slog.NewLogLogger`/`slog.NewRecord`, `AddAttrs`/`WithAttrs`, `Handler().Handle(...)`, a logger method taken as a value, `panic` with a non-literal argument, and any declaration shadowing the `obs` import. The count of **7** sites guards against the walker going blind, not against a shape it does not recognise. It CANNOT see a helper in another package, reflection or unsafe, or an output channel not named here. `TestTheLogSiteCheckerCatchesPlantedUnredactedCalls` plants each refused shape and requires the checker to report it (security review of core #8, N-7 / F-1; B3 verifier, core #12, V-1 / V-2) |
...
| Every 503 on the setup surface logs its cause once, redacted, with the request id — the claim-status GET and the unclaimed guard included, which used to log only the canned message (sentinel S-0006). The ONE thing not logged, here or by the shared error handler, is the request's own cancellation: the cause is `context.Canceled` AND the request's context is done (the client hung up; sentinel S-0005, RULES R9). Nothing wider (sentinel PR #14 F-4, verifier F-2): a `DeadlineExceeded` cause on a live request — a pgx connect timeout — is logged as an outage, and any 5xx that merely coincided with the client leaving (a reverse proxy timing out on a real defect cancels the request context too) is logged by the error handler. `ownerclaim.unavailable` wraps its cause with `%w` so this is decidable (RULES R18). **Residual:** a database that HANGS while every client or proxy gives up first yields cancelled requests only, which are not logged; `/readyz` and `vizra doctor` still report the database | `s.unavailable`, `isOwnCancellation`, `errorHandler`; `TestEverySetup503LogsItsCause` (claim-status, a live-request connect timeout, the guard, and a POST with no pool), `TestACancelledClaimIsNotLoggedAsADatabaseOutage` (cancelled during the claimed check, and inside the claim transaction while the redeem waits on a held row lock), `TestAFiveHundredIsLoggedEvenWhenTheClientHasGone`, `TestTheRequestsOwnCancellationIsNotLoggedAsAFailure` |
| The owner-claim validator never accepts an input the database then refuses: the email expression is the DDL's own literal (`TestValidatorsMatchTheMigration` compares it byte for byte, the username too), and because PostgreSQL's `[:space:]` follows the libc locale while Go's is ASCII-only, the validator ALSO refuses every Unicode space and control character and invalid UTF-8. That is a superset of PostgreSQL's `[:space:]` under `en_US.utf8`, measured EXHAUSTIVELY (every code point) on every run. A value `text` cannot hold (NUL, SQLSTATE 22021) is mapped to 400 as a backstop (sentinel S-0003, RULES R17). **At exactly this strength:** other libc or ICU locales are an argument, not a measurement — one that classed a code point Go does not refuse (a Cf such as U+180E or U+200B) as space would reach the `users_email_shape` backstop, a 400, never a 5xx. Invalid UTF-8 is refused by `Validate` but cannot arrive over HTTP (encoding/json substitutes U+FFFD). Only the input CHECKs the validator mirrors (`users_username_shape`, `users_email_shape`) are answered 400; **any other 23514 is a server defect: 500, logged with its redacted cause** — it used to be told to the operator as bad input (sentinel PR #14 F-1) | `TestEveryEmailTheValidatorAcceptsTheDatabaseAccepts` (one corpus through Go and through users_email_shape's CHECK expression read from 0005's bytes, evaluated by PostgreSQL), `TestEveryCodePointThisServerCallsSpaceIsRefusedByTheValidator` (exhaustive), `TestAnEmailTheDatabaseCannotHoldIsA400NotA500`, `TestTheEmailValidatorRefusesEveryLocaleDependentSpaceAndControl`, `TestTheEmailValidatorRefusesInvalidUTF8`, `TestTheEmailRefusalSaysWhatIsWrong`, `TestClaimErrorMapping` (22021; an input 23514 is 400, any other 23514 is 500), `TestANonInputCheckViolationIsAServerErrorNotA400` |
| The claim body is decoded STRICTLY: exactly one JSON object, keys exactly the four the schema defines (case-sensitive; `encoding/json` alone matches case-insensitively), and nothing after the object but whitespace (`dec.More()` alone accepts a trailing `}` or `]`) (sentinel S-0009) | `decodeClaimOwnerRequest`; `TestTheClaimBodyIsDecodedStrictly` |
| A `setup.owner_claim.succeeded` audit row names the token generation it redeemed (`after.token_generation`), as 0005 promises (sentinel S-0007). **The schema does not require it yet, deliberately (expand/contract, RULES R22):** during a rolling deploy the PREVIOUS binary still claims against the new schema and writes `{username, role}` only, so a CHECK requiring the key would fail every claim it serves (sentinel PR #14 F-1, measured with the two real binaries). **Queued:** a later migration adds that CHECK once no binary that omits the key can be serving; until then `TestTheSchemaStillAcceptsThePreviousBinarysSucceededRow` holds the N-1 shape open, and it is deleted in the same change that adds the CHECK | `TestTheSucceededRowNamesTheConsumedGeneration`, `TestTheSchemaStillAcceptsThePreviousBinarysSucceededRow` |
| `VIZRA_OWNER_CLAIM_TTL` is at least 1m at config load. Below that, the microsecond-truncated interval failed `owner_claim_tokens_ttl` on every mint, or left too little time to use the token (sentinel S-0010). There is no upper bound | `config.MinOwnerClaimTTL`, `TestTheOwnerClaimTTLHasALowerBound` |
...
| "Never mint on a claimed instance" is a property of the STATEMENT, not of its callers — including a re-mint that WAITS on a claim's row lock. The `NOT EXISTS (SELECT 1 FROM users)` guard is read in the statement's snapshot, so on its own it let `vizra claim-token`, blocked on an in-flight claim, run its `DO UPDATE` against the committed CONSUMED row after the wait: a live token on a claimed instance and the consumption record erased (sentinel S-0002). The `DO UPDATE`'s own `WHERE owner_claim_tokens.consumed_at IS NULL` is evaluated against the locked, latest row version, so the statement returns no row and `Mint` returns `ErrHasUsers` (RULES R16) | `MintOwnerClaimToken`; `TestMintIsRefusedByTheDatabaseOnAClaimedInstance` (direct store call), `TestARemintRacingAClaimDoesNotMintOnTheClaimedInstance` (the claim's redeem held open, the re-mint blocked on it, then COMMIT), `TestARemintDuringARealHTTPClaimMintsNothing` (the real handler; a test-only trigger sleeps in the claim's `succeeded` insert) |
...
| A rate-limited claim writes no audit row PER REQUEST. Exactly ONE `rate_limited` row is written per bucket per window, on that bucket's transition into the limited state, and `after.bucket` names it: `per_origin`, `global`, `ceiling.claim` or `ceiling.status`. The marker is per bucket KIND (never per origin), so at most four rows per window **with a shared cache**; while the limiter runs on its in-process fallback the marker is per process, so it is at most four per window PER API PROCESS (the verifier measured 8 from two processes). **Accepted residual, stated:** an instance left UNCLAIMED under a sustained flood accumulates up to four rows per 15 minutes (384 a day, per process while the cache is down) for as long as it stays unclaimed. The hard ceilings are included (sentinel S-0004), so a ceiling 429 that refused the operator's VALID token is in the trail — but only while the instance is UNCLAIMED: on a claimed instance the ceilings still answer 429 and write nothing, because one row per window for the life of an instance is still an unbounded writer into an undeletable table | `TestARateLimitedClaimWritesNoAuditRow` (0 rows before a transition; exactly one each for `per_origin`, `global` and, after ten times more traffic, `ceiling.claim`; never a second per bucket), `TestPerOriginAndGlobalTransitionsEachWriteTheirOwnRow`, `TestTheClaimCeilingRefusingTheValidTokenIsAudited`, `TestTheStatusCeilingIsAuditedOnce` |
| A request carrying the VALID claim token is never answered 429 by the failure limiter. **The per-route HARD CEILING is different, and CAN answer a valid token 429:** each setup route has its own fixed-window counter — claim-owner **600** and claim-status **3000** requests per **15 minutes** (`claimOwnerCeiling`, `claimStatusCeiling`, `internal/httpapi/setup.go`) — and once a route's count is reached every request to that route, the operator's valid-token claim included, is 429 until the window rolls. The window is FIXED: it starts at the first request and its end never moves (see the limiter row below), so the lockout ends at most 15 minutes after the window opened, whatever arrives in between. The two routes never share a bucket, so a status flood cannot spend the claim's budget. **Accepted residual, at exactly this strength**, recorded here and at `allowSetupRequest` (`internal/httpapi/setup_limits.go`): an attacker who sends **600 claim-owner requests in every 15-minute window** (about 0.67 per second, sustained) holds claim-owner closed to everyone, the operator included, for as long as they keep it up. The guard the pool actually wants is a concurrency bound, which is M1-B's; until it lands this stands. (Before verifier R4-A the cache path refreshed the window on every request, so after ONE burst a single request per window held the lockout for ever; that is fixed.) | `TestAValidTokenIsNeverRateLimitedByTheFailureLimiter`; the ceiling: `TestClaimStatusIsBoundedByTheHardCeiling` (MUT-34), `TestTheTwoSetupRoutesDoNotShareAHardCeilingBucket` (MUT-41), `TestAStatusFloodDoesNotStarveTheClaim`, `TestAClaimFloodDoesNotSilenceTheStatusEndpoint` |
| The rate limiter is a **FIXED window on every path**: the cache path sets the key's TTL when the window STARTS and never moves it, and a counter found with NO TTL is given one on its next call, so no key can become a permanent lockout. The cache-down fallback and the in-process limiter make the same decisions on the same script. Every bucket inherits this: the claim-owner ceiling (600/15 min), the claim-status ceiling (3000/15 min), the failure budget per origin (10/15 min) and global (60/15 min), and the one-per-window `rate_limited` audit transition (1/15 min). Previously `INCR` + an unconditional `EXPIRE` refreshed the window on every call — refused requests included — which made each bucket a sliding lockout that one request per window could hold closed indefinitely (verifier R4-A, measured through the real handler on Valkey). | `FallbackLimiter.Allow`: `INCR` + `EXPIRE key window NX` in one MULTI/EXEC (not interleaved with other clients; a crash never leaves one applied without the other (client crash before EXEC: neither; after EXEC: both; a half-written AOF tail is truncated on load); a queue-time error discards both). A runtime error is NOT rolled back, but the only realistic one, INCR on a non-integer, still leaves EXPIRE NX to set the TTL, and a counter found without a TTL is given one on its next call, so no path leaves a permanent lockout (NX needs Redis >= 7.0; ADR-001 sets the floor at Redis/Valkey >= 7.2). `TestTheCacheLimiterDoesNotRefreshTheTTLWithinTheWindow`, `TestTheCacheLimiterReopensWhenTheWindowRolls`, `TestTheCacheLimiterGivesATTLToAKeyFoundWithoutOne`, `TestTheCacheAndFallbackLimitersAgreeOnFixedWindowSemantics` (integration, run on both CI cache images); MUT-62 (the unconditional EXPIRE restored), MUT-63 (the TTL set only when INCR returns 1) |
| A request whose context has ENDED is not a cache failure: the limiter answers it `(false, 0)` and touches neither the cache, the in-process counter nor the degraded flag — checked before the command and again when the command fails, since a context that ends mid-command surfaces as an `Exec` error. Before this, a client hanging up flipped `/readyz` to degraded and charged the fallback for a dead request (limiter review L-2). TODAY only a client disconnect ends a request context before `Allow`: the server's Read/WriteTimeout do not cancel it (B3 verifier V-3, measured) and no per-request deadline exists. If one is added, a still-waiting client whose deadline expired would be answered 429 by the ceiling callers, so they must map a context error first | `FallbackLimiter.Allow`; `TestACancelledRequestDoesNotFlipTheLimiterToDegraded` (with a live-context control that DOES flip, against the same dead cache), `TestARequestCancelledMidCommandDoesNotFlipTheLimiterToDegraded`, `TestACancelledRequestDoesNotChargeTheInProcessLimiter` |
| The claim transaction pins READ COMMITTED explicitly, so the race holds whatever `default_transaction_isolation` the server is set to | `TestOwnerClaimRaceYieldsExactlyOneOwnerUnderEveryServerDefaultIsolation` (32 claimants × 3 server defaults; asserts the SERVER's own pool runs under each default, exactly one 201, 31 × 409, zero 5xx, one row each in users / credentials / `succeeded`, and zero `refused` rows or failure-budget charges from the losers). Its one CI failure (run 35814919455) was the read-phase classification defect (the "A token refusal is answered 403 only if" row), not timing; the stress runs behind that statement are in `docs/evidence/m1a-owner-claim/07-race-stress.txt` |
