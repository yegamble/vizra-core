...
`ci-required-guard` runs ten checks over `set(FLOOR_LANES) | set(required)`.
Check **8b** reads each make step's `run:` as shell and refuses no-op flags,
`VAR=value` overrides and a MAKEFLAGS-family `env:`; check **9** requires a
make-free invocation of BOTH suites; check **10** requires a provenance step in
every checked lane that checks out. Its own negative cases are
`scripts/testdata/guard/` (44 fixtures), driven by `scripts/scripts_test.go`.
...
fails on any skip not allowlisted by test name **with a reason**, fails below
the floors recorded in `scripts/test-floors.json` (whole-suite and per-package),
and prints the counts. The recipes keep their own required coverage in both
`cache-matrix` legs.
...
`chore/m0-hardening-b1`. **This machine is not the ADR-009 acceptance platform;
CI `ubuntu-24.04` is.**
...
| `go test -race -count=1 ./scripts/` | 0 | the guard, report, image and migrate-lint fixtures |
